...
- 🔲 Phases 7-8 not started (forced password change, wrap-up)
...
**Test count as of Phase 6**: 334/334 passing, PHPStan 0 errors, Pint clean.
...
**Immediate next step:** resume the professor revisions (see that section above) on branch `feature/professor-revisions`. Phases 1-6 are done; **Phase 7 (login by username or email, forced password change) is next**, and its plan must be presented and approved first. Read `c:\Users\Rem\Downloads\REVISIONS_PROMPT.md` first if it is not still in context.
