...
- **Guaranteed scheduling in production** — `schedule:work` runs inside the container, but the free instance sleeps after 15 minutes idle, so a run can be skipped while it is asleep. The commands are idempotent, so run `invoices:generate` from the Shell tab if a day was missed.
...
**Immediate next step:** the professor revisions are complete on `feature/professor-revisions`. The remaining decision is the user's: merge to `main` (auto-deploys; do not merge PR #1). After the deploy: read the logs for the new migrations and confirm the `PHP_CLI_SERVER_WORKERS` warning is gone, run `php artisan app:create-super-admin` in Render's Shell, then register a test landlord with an ID and confirm the file appears in the `occuplace-sensitive` bucket and survives a redeploy.
